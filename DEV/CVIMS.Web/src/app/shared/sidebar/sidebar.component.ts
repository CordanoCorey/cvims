...
export class SidebarComponent extends SmartComponent implements OnInit {
  ...
  get onProductsSearch(): boolean {
    return this.inProducts && !this.onProduct || this.onSchoolStoreHome || this.onSupplyStoreHome || this.onWarehouseHome;
  }
  ...
  refresh() {
    // console.log(
    //   `\nURL:\t${this.url}`,
    //   `\nIn Products:\t${this.inProducts}`,
    //   `\nOn Product:\t${this.onProduct}`,
    //   `\nOn Products:\t${this.onProducts}`,
    //   `\nOn Products Search:\t${this.onProductsSearch}`
    // );
    setTimeout(() => {
      if (this.onDashboard) {
        this.openSidenav();
      } else if (this.onProductsSearch || this.inOrders) {
        this.closeSidenav();
      }
    }, 0);
  }
